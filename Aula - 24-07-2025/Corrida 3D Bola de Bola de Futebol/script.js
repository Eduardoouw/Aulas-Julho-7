...
const grassTexture = textureLoader.load('textures/Grama/Grass1.jpg', 
    () => console.log('Textura Grass1.jpg carregada com sucesso'),
    undefined,
    (err) => console.error('Erro ao carregar Grass1.jpg:', err)
);
const grassNormal = textureLoader.load('textures/Grama/Grass2.jpg', 
    () => console.log('Textura Grass2.jpg carregada com sucesso'),
    undefined,
    (err) => console.error('Erro ao carregar Grass2.jpg:', err)
);
grassTexture.wrapS = grassTexture.wrapT = THREE.RepeatWrapping;
grassTexture.repeat.set(10, 20);
grassNormal.wrapS = grassNormal.wrapT = THREE.RepeatWrapping;
grassNormal.repeat.set(10, 20);
const planeGeometry = new THREE.PlaneGeometry(50, 1000);
...
// 4. Criar o Jogador (Esfera com textura de bola de futebol)
const playerTexture = textureLoader.load('img/bola.jpg', 
    () => console.log('Textura bola.jpg carregada com sucesso'),
    undefined,
    (err) => console.error('Erro ao carregar bola.jpg:', err)
);
...
let gameSpeed = 0.2; // Velocidade de avanço do jogo
...
const obstacleSpawnInterval = 60; // frames entre cada obstáculo
...
function animate() {
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        const playerHalfSize = 0.5; // Raio da esfera ou cilindro
        const obsHalfSize = 0.5;
        if (
    ...
    if (obstacleSpawnCounter >= obstacleSpawnInterval) {
        ...
        obs.position.set(
            ...
            0.5,
            player.position.z - 60 // sempre à frente do jogador
        );
        scene.add(obs);
        obstacles.push(obs);
    }
